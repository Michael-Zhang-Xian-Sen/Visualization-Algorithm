...
function inputCreate() {
	...
	for (i = 0; i < inputArray.length; i++) {
		...
		if (parseInt(inputArray[i]) > 50 || parseInt(inputArray[i]) < 1) {
			alert("请确保每个数据的大小介于1~50之间。");
			return false;
		}
...
function createAlgorithmDirection() {
	...
	for (var i = 0; i < sort_code.length; i++) {
		...
		if(parseInt(window.innerWidth) >= 1919 || parseInt(window.innerWidth) <= 769){
			...
			p[i].style.padding = "0px 0px 0px 10px";
		}
...
function result() {
	...
	if (upflag == 1 && up_sorted_flag == 0) quickSortUp(delayTime);
